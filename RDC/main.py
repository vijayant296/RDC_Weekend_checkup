...
class Application(db.Model):
    ...
    app_date = db.Column(db.DateTime, nullable=True, default = datetime.now())
...
# recovered = 'https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_recovered_global.cs'
...
@app.route("/status-app", methods=['GET', 'POST'])
def appstat():
        ...
        if request.method == 'POST':
            for app in applications:
                ...
                if app.app_status == application_status:
                    ...
                    app.app_date = datetime.now()
                    db.session.commit()
            applications = Application.query.all()
            return redirect('updated-status')
        return render_template('status_apps.html', applications=applications, params=params)
    else:
        return redirect(url_for('login'))


@app.route("/updated-status", methods=['GET', 'POST'])
